""" Get the accuracies of the models defined in ../models.py, for each candidate corruption (using the ImageNet validation set corrupted with the considered corruption)
...
corruption_list = list(candidate_corruptions.dict_corruptions.keys())
...
for i in range(len(model_to_test)):
    ...
    for j in range(len(corruption_list)):
        ...
        res_array[i,j] = test_acc.item()
        test_acc = 0

res_array = pandas.DataFrame(res_array, index=model_to_test, columns=corruption_list)
res_array.to_pickle(os.path.join("../Results/benchmark_correlations","inet_all_candidates_accuracies.pkl"))
res_array.to_html(os.path.join("../Results/benchmark_correlations","inet_all_candidates_accuracies.html"))
